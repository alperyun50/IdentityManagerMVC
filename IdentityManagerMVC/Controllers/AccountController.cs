...
using IdentityManagerMVC.Controllers;
...
namespace IdentityManager.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            //if is role not exist in database
        ...
        {
            // delete automatic login redirection from privacy page
            // and delete extra url (?ReturnUrl=%2FHome%2FPrivacy)
            ViewData["ReturnUrl"] = returnurl;
            return View();
        }
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnurl=null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    // use LocalRedirect for security
                    return LocalRedirect(returnurl);
                }
        ...
        {
            // request a redirect to the external login provider (facebook)
            var redirecturl = Url.Action("ExternalLoginCallback", "Account", new { ReturnUrl = returnurl });
            var properties = _signInManager.ConfigureExternalAuthenticationProperties(provider, redirecturl);

            return Challenge(properties, redirecturl);
        }
           ...
           if(ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("Verify", "Tour two factor auth code could not be avalidated.");
                    return View(model);
                }
            }
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<IActionResult> VerifyAuthenticatorCode(bool remebmerMe, string returnUrl = null)
        {
            ...
            ViewData["ReturnUrl"] = returnUrl;
            return View(new VerifyAuthenticatorViewModel { ReturnUrl = returnUrl, RememberMe = remebmerMe});
        }
